...
import 'package:gameopolis/games/runner/background/cloud.dart';
import 'package:gameopolis/games/runner/random_extension.dart';
import 'package:gameopolis/games/runner/runner_game.dart';

class CloudManager extends PositionComponent with HasGameRef<RunnerGame> {
  ...
  void update(double dt) {
    ...
    if (numClouds > 0) {
      ...
      if (numClouds < maxClouds && (gameRef.size.x / 2 - lastCloud.x) > lastCloud.cloudGap) {
        addCloud();
      }
    } else {
      addCloud();
